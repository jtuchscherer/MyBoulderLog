...
import com.google.gwt.requestfactory.rebind.RequestFactoryGenerator;
...
public class FixedRequestFactoryGenerator extends Generator {
    ...
    public String generate(
            TreeLogger logger, GeneratorContext context, String typeName) throws UnableToCompleteException
    {
    ...
    private void writeAutoBeanFactory(SourceWriter sw) {
        ...
        sw.println("@%s({%s.class, %s.class, %s.class})",
                   AutoBeanFactory.Category.class.getCanonicalName(),
        ...
        sw.println("@%s(%s.class)",
                   AutoBeanFactory.NoWrap.class.getCanonicalName(),
        ...
        sw.println("public static final Factory FACTORY=%s.create(Factory.class);", GWT.class.getCanonicalName());
            ...
            for (RequestMethod request : method.getRequestMethods()) {
                ...
                for (JParameter param : jmethod.getParameters()) {
                    ...
                    parameterDeclaration.append(",final ")
                            .append(param.getType().getParameterizedQualifiedSourceName())
                            .append(" ")
                ...
                String elementType =
                        request.isCollectionType() ? request.getCollectionElementType().getQualifiedSourceName() +
                                ".class" : "null";
    ...
    private void writeTypeMap(SourceWriter sw) {
        ...
        for (EntityProxyModel type : model.getAllProxyModels()) {
            ...
            sw.println("%s.add(%s.class);",
                       type.getType().equals(EntityProxyModel.Type.ENTITY) ? "entityProxyTypes" : "valueProxyTypes",
                       type.getQualifiedSourceName()
            );
        }
        sw.outdent();
